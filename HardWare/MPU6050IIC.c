...
#define IIC_SDA_PIN       GPIO_PIN_12                  /* 连接到SDA数据线的GPIO */
...
#define IIC_SDA_READ()  HAL_GPIO_ReadPin(MPU6050_SDA_GPIO_Port, MPU6050_SDA_Pin)	/* 读SDA口线状态 */
...
uint8_t MPU6050IIC_CheckDevice(uint8_t _Address)
{
    ...
    MPU6050IIC_GPIO_Init();		/* 配置GPIO */

    MPU6050IIC_Start();		/* 发送启动信号 */

    /* 发送设备地址+读写控制bit（0 = w， 1 = r) bit7 先传 */
    MPU6050IIC_Send_Byte(_Address|IIC_WR);
    ucAck = MPU6050IIC_Wait_Ack();	/* 检测设备的ACK应答 */

    MPU6050IIC_Stop();			/* 发送停止信号 */
